...
export function GeneralEmployeePanel({ employees, loadingEmployees, departments, loadingDepartments, onFormChange }: Props) {
    ...
    <div>
        ...
        {recipientMode === 'individual' ? (
          <Grid2>
            ...
            <Field label="Employee ID">
              <Input value={form.employee_id_display} readOnly placeholder="Auto-filled" className="opacity-60" />
            </Field>
            <Field label="Employee email" hint="Auto-filled from selection">
              <Input value={form.employee_email} readOnly placeholder="Auto-filled" />
            </Field>
          </Grid2>
        ) : (
          <div className="flex flex-col gap-3">
            <Field label="Recipient group">
              <Select value={selectedGroup} onChange={(e) => { setSelectedGroup(e.target.value); setSelectedDepartmentId('') }}>
            ...
            {selectedGroup === 'Department-wise' && (
              <Field label="Select department" hint="Fetched from /core/departments">
                <select
      ...
      <Card title="Mail content">
        <Field label="Subject / purpose">
          <Input value={form.subject} onChange={set('subject')} placeholder="e.g. Q2 Company All-hands — Save the Date" />
        </Field>
        ...
        <Field label="Attachments note">
          <Input value={form.attachment_note} onChange={set('attachment_note')} placeholder="e.g. Please find the updated policy document attached" />
        </Field>
        <Field label="Action required by employee">
          <Textarea value={form.action_required} onChange={set('action_required')} rows={2} placeholder="e.g. Please acknowledge this email by EOD 30 May 2026." />
        </Field>
        <Field label="Closing remarks">
          <Textarea value={form.closing_remarks} onChange={set('closing_remarks')} rows={2} placeholder="e.g. Thank you for your continued dedication." />
        </Field>
      </Card>
